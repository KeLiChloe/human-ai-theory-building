...
def legend_entry(
    ...
    *,
    ...
    n_token = r"$\mathit{N}$" if italic_uppercase_n else "n"
    eq = " = " if italic_uppercase_n else "="
    if include_composition and group_key == "Human":
        return f"{label} ({n_token}{eq}{n}, {HUMAN_COMPOSITION_NOTE})"
    return f"{label} ({n_token}{eq}{n})"
