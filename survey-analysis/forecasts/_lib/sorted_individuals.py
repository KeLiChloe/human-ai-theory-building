...
Outputs:
  forecasts/outputs/fig_sorted_individuals.{svg,png}           (Humans pooled)
  forecasts/outputs/fig_sorted_individuals_3group.{svg,png}    (Senior / PhD / GenAI)
"""
...
OUT_PATH_3GROUP = FORECASTS / "outputs" / "fig_sorted_individuals_3group.png"
...
# Mid saturation for the dense 3-group scatter (between shared pastels and vivid).
GROUP_COLORS_3GROUP = {
    "senior": "#F5A965",
    "phd": "#5CA3E2",
    "genai": "#4BB25C",
...
def _draw_panel(
    ...
    *,
    ...
    group_colors = GROUP_COLORS if collapsed else GROUP_COLORS_3GROUP
    ...
    if collapsed:
        ...
        ax.scatter(
            x[is_senior], y[is_senior],
            c=group_colors["senior"], marker="o", s=28,
            alpha=1.0, edgecolors=edge, linewidths=lw, zorder=5,
        )
        ax.scatter(
            x[is_phd], y[is_phd],
            c=group_colors["phd"], marker="o", s=28,
            alpha=1.0, edgecolors=edge, linewidths=lw, zorder=5,
        )
    ax.scatter(
        x[is_gen], y[is_gen],
    ...
    lg = legend_fn(
        ...
        collapsed=collapsed, group_means=group_means,
    )
    ...
    lg.remove()
    if not collapsed:
        # Keep legend swatches in sync with the brighter 3-group palette.
        for h, t in zip(handles, labels):
            if t.startswith("Senior Scientists"):
                h.set_color(group_colors["senior"])
            elif t.startswith("PhD Students"):
                h.set_color(group_colors["phd"])
            elif t.startswith("GenAI (mean") or t.startswith("Aggregated GenAI"):
                h.set_color(group_colors["genai"])
...
def plot_sorted_individuals_both(
    out_collapsed: Path = OUT_PATH,
    out_3group: Path = OUT_PATH_3GROUP,
) -> tuple[Path, Path]:
    """Write Humans-pooled and Senior/PhD/GenAI versions."""
    p1 = plot_sorted_individuals(out_collapsed, collapsed=True)
    p2 = plot_sorted_individuals(out_3group, collapsed=False)
    return p1, p2


def main() -> None:
    for path in plot_sorted_individuals_both():
