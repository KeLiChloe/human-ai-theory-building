...
def _domain_specs(base: Path) -> dict[str, dict]:
    return {
        "race": {
            ...
            "jobs": [
                {
                    "rf_json": base / "race/main_effects/RF_feature_importance_results_mdi.json",
                ...
                {
                    "rf_json": base / "race/soi/RF_feature_importance_results_mdi_soi.json",
        ...
        "gender": {
            ...
            "jobs": [
                {
                    "rf_json": base / "gender/main_effects/RF_feature_importance_results_mdi.json",
                ...
                {
                    "rf_json": base / "gender/soi/RF_feature_importance_results_mdi_soi.json",
...
def _draw_rf_classification_on_ax(ax, payload, show_legend: bool = True):
    ...
    ax.plot(
        thresholds, payload["accuracy"], label="Accuracy",
        color="#D5006A", linewidth=4.2, linestyle="-",
    )
    ax.plot(
        thresholds, payload["precision"], label="Precision",
        color="#0077BB", linewidth=4.2, linestyle=(0, (9, 3.5)),
    )
    ax.plot(
        thresholds, payload["recall"], label="Recall",
        color="#00C853", linewidth=4.2, linestyle=(0, (8, 2.5, 2.2, 2.5)),
    )
    ax.plot(
        thresholds, payload["f1"], label="F1 Score",
        color="#111111", linewidth=4.2, linestyle=(0, (2.4, 2.4)),
    )
...
def plot_domain_figure(
    ...
    for i, row in enumerate(loaded):
        ...
        rf_block = panels[0, 0].subgridspec(
            1, 2, width_ratios=[1.0, 0.048], wspace=0.10
        )
        ...
        draw_feature_importance_on_ax(
            ...
            bar_label_size=110 if is_soi else 85,
        )
    ...
    for ax in (*rf_axes, *lr_axes, *cbar_axes, ax_c):
